	...
	dfs = func(i, left int) int {
		...
		if memo[left][i] == 0 {
			...
			for j := i + 1; j < n; j++ {
				...
				if profit > 0 {
					result = max(result, profit+dfs(j+1, left-1))
				}
			}

			result = max(result, dfs(i+1, left))
			memo[left][i] = result
		}
		return memo[left][i]
	}
